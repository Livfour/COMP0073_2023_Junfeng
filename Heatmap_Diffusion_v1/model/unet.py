...
class NaiveUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        down1_output = self.down1(x)
        # print(f"down1 shape: {down1_output.shape}")
        down2_output = self.down2(down1_output)
        # print(f"down2 shape: {down2_output.shape}")
        down3_output = self.down3(down2_output)
        # print(f"down3 shape: {down3_output.shape}")

        thro = self.to_vec(down3_output)
        # print(f"thro shape: {thro.shape}")
        temb = self.timeembed(t).view(-1, self.n_feat * 2, 1, 1)
        # print(f"temb shape: {temb.shape}")
        thro = self.up0(thro + temb)
        # print(f"thro shape: {thro.shape}")
        up1_output = self.up1(thro, down3_output) + temb
        # print(f"up1 shape: {up1_output.shape}")
        up2_output = self.up2(up1_output, down2_output)
        # print(f"up2 shape: {up2_output.shape}")
        up3_output = self.up3(up2_output, down1_output)
        # print(f"up3 shape: {up3_output.shape}")
        out = self.out(torch.cat((up3_output, x), 1))
