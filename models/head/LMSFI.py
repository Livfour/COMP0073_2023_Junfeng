...
class LMSFI(BaseHead):
    def __init__(self,
        ...
        self.down4 = nn.MaxPool2d(kernel_size=4, stride=4)
        self.conv_in_1 = nn.Conv2d(
            in_channels=num_keypoints, out_channels=num_keypoints, kernel_size=3, stride=1, padding=1)
        self.conv_in_2 = nn.Conv2d(
            in_channels=num_keypoints, out_channels=num_keypoints, kernel_size=3, stride=1, padding=1)
        self.conv_in_3 = nn.Conv2d(
            in_channels=num_keypoints, out_channels=num_keypoints, kernel_size=3, stride=1, padding=1)
        self.conv_out_1 = nn.Conv2d(in_channels=2*emb_dim, out_channels=256,
                                    kernel_size=3, stride=1, padding=1)
        self.conv_out_2 = nn.Conv2d(in_channels=2*emb_dim, out_channels=256,
                                    kernel_size=3, stride=1, padding=1)
        self.conv_out_3 = nn.Conv2d(in_channels=2*emb_dim, out_channels=256,
                                    kernel_size=3, stride=1, padding=1)
    ...
    def _fusion(self, f_t, h_t, conv_in, conv_out, up):
        h_t_bar = conv_in(h_t)
        h_t_bar = self._squeeze(h_t_bar)
        h_t_bar = self.sigmod(h_t_bar)
        f_t_masked = f_t * h_t_bar
        f_t_out = torch.cat([f_t_masked, f_t], dim=1)
        f_t_out = conv_out(f_t_out)
        f_t_out = up(f_t_out)
        return f_t_out
    ...
    def forward(self, f, h, _ts=None):
        ...
        f_2 = self.up2(f)
        f_3 = self.up4(f)
        out_1 = self._fusion(f_1, h_1, self.conv_in_1,
                             self.conv_out_1, self.up4)
        out_2 = self._fusion(f_2, h_2, self.conv_in_2,
                             self.conv_out_2, self.up2)
        out_3 = self._fusion(f_3, h_3, self.conv_in_3,
                             self.conv_out_3, self.up1)
