...
class MaskHead2(BaseHead):
    def __init__(self,
        ...
        self.out = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(
                d_model=768*2, nhead=8),
            num_layers=1,
        )
    ...
    def forward(self, f, h, _ts=None):
        ...
        f_t = f_t.permute(0, 2, 3, 1).reshape(B, -1, self.emb_dim*2)
        out = self.out(f_t)
        out = out[:, :, :self.emb_dim]
        out = out.reshape(B, self.emb_dim, 16, 12)
